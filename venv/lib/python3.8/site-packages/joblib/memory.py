...
def _store_backend_factory(backend, location, verbose=0, backend_options=None):
    ...
    if isinstance(location, StoreBackendBase):
        ...
        warnings.warn(
            f"Instanciating a backend using a {location.__class__.__name__} as a location is not supported by joblib. Returning None instead.",
            UserWarning,
        )
...
def _format_load_msg(func_id, args_id, timestamp=None, metadata=None):
    ...
    try:
        if metadata is not None:
            ...
            signature = f"{os.path.basename(func_id)}({args})"
        else:
...
class MemorizedFunc(Logger):
        ...
        if not (self._check_previous_func_code(stacklevel=4) and
            ...
            try:
                ...
                out = (
                    None
                    if shelving
                    else self.store_backend.load_item(
                ...
                if self._verbose > 4:
                    t = time.time() - t0
                    _, name = get_func_name(self.func)
                    msg = f'{name} cache loaded - {format_time(t)}'
    ...
    def _check_previous_func_code(self, stacklevel=2):
        ...
        if old_first_line == first_line == -1 or func_name == '<lambda>':
            func_description = (
                func_name
                if first_line == -1
                else ("{0} ({1}:{2})".format(func_name, source_file, first_line))
            )
    ...
    def call(self, *args, **kwargs):
        ...
        if self._verbose > 0:
            _, name = get_func_name(self.func)
            msg = f'{name} - {format_time(duration)}'
            print(max(0, (80 - len(msg))) * '_' + msg)
...
class Memory(Logger):
    ...
    def cachedir(self):
        warnings.warn(
            ...
            DeprecationWarning, stacklevel=2)
        return None if self.location is None else os.path.join(self.location, 'joblib')
