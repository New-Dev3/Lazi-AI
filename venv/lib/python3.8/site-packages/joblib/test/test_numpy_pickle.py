...
_complex = complex(1)
_string = str(1)
typelist = [_none, _type, _bool, _int, _float, _complex, _string]
_tuple = ()
...
def test_numpy_persistence(tmpdir, compress):
    ...
    for obj in (np.matrix(np.zeros(10)), np.memmap(f'{filename}mmap', mode='w+', shape=4, dtype=np.float)):
        filenames = numpy_pickle.dump(obj, filename, compress=compress)
        # All is cached in one file
        assert len(filenames) == 1
...
def test_memmap_persistence(tmpdir):
    ...
    obj_loaded.array_float[:10] = 10.0
    assert obj_loaded.array_int.flags.writeable
    obj_loaded.array_int[:10] = 10
...
def _check_pickle(filename, expected_list):
    ...
    py_version_used_for_writing = int(version_match[1])
...
@parametrize('compress_tuple,message', [(('zlib', 3, 'extra'),        # wrong compress tuple
               'Compress argument tuple should contain exactly 2 elements'), (('wrong', 3), 'Non valid compression method given: "wrong"'), (('zlib', 'wrong'), 'Non valid compress level given: "wrong"')])
...
@parametrize('data', [b'a little data as bytes.', (10000 * f"{random.randint(0, 1000) * 1000}".encode('latin-1'))], ids=["a little data as bytes.", "a large data as bytes."])
...
def test_binary_zlibfile_bad_compression_levels(tmpdir, bad_value):
    ...
    pattern = re.escape(
        f"'compresslevel' must be an integer between 1 and 9. You provided 'compresslevel={bad_value}'"
    )
