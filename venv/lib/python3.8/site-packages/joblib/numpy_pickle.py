...
def dump(value, filename, compress=0, protocol=None, cache_size=None):
    ...
    if compress is True:
        ...
        if len(compress) != 2:
            raise ValueError(
                f'Compress argument tuple should contain exactly 2 elements: (compress method, compress level), you passed {compress}'
            )
    ...
    if not is_filename and not is_fileobj:
        ...
        # incomprehensible
        raise ValueError(
            f'Second argument should be a filename or a file-like object, {filename} (type {type(filename)}) was given.'
        )
    ...
    if cache_size is not None:
        ...
        warnings.warn(
            f"Please do not set 'cache_size' in joblib.dump, this parameter has no effect and will be removed. You used 'cache_size={cache_size}'",
            DeprecationWarning,
            stacklevel=2,
        )
