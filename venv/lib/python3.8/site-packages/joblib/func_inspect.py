...
def filter_args(func, ignore_lst, args=(), kwargs=dict()):
    ...
    if isinstance(ignore_lst, str):
        ...
        raise ValueError(
            f'ignore_lst must be a list of parameters to ignore {ignore_lst} (type {type(ignore_lst)}) was given'
        )
    args = list(args)
    ...
    for arg_name, arg_value in sorted(kwargs.items()):
        if arg_name in arg_dict:
            ...
            raise TypeError(
                f"Ignore list for {name}() contains an unexpected keyword argument '{arg_name}'"
            )
    ...
    for item in ignore_lst:
        if item in arg_dict:
            ...
            raise ValueError(
                f"Ignore list: argument '{item}' is not defined for function {_signature_str(name, arg_sig)}"
            )
