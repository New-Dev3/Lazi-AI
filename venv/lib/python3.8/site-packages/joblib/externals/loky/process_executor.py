...
def _python_exit():
    ...
    mp.util.debug(
        f"Interpreter shutting down. Waking up executor_manager_thread {items}"
    )
...
def _get_chunks(chunksize, *iterables):
    ...
    if sys.version_info < (3, 3):
        ...
        it = zip(*iterables)
    while True:
        if chunk := tuple(itertools.islice(it, chunksize)):
            yield chunk
        else:
            return
...
class _ExecutorManagerThread(threading.Thread):
    ...
    def wait_result_broken_or_wakeup(self):
        ...
        if result_reader in ready:
            ...
            if sys.platform != "win32":
                ...
                exit_codes = ''
            mp.util.debug(
                f'A worker unexpectedly terminated. Workers that might have caused the breakage: {{p.name: p.exitcode for p in list(self.processes.values()) if p is not None and p.sentinel in ready}}'
            )
            bpe = TerminatedWorkerError(
                f"A worker process managed by the executor was unexpectedly terminated. This could be caused by a segmentation fault while calling the function or by an excessive memory usage causing the Operating System to kill the worker.\n{exit_codes}"
            )
    ...
    def join_executor_internals(self):
        ...
        mp.util.debug(
            f"executor management thread clean shutdown of worker processes: {list(self.processes)}"
        )
...
def _check_max_depth(context):
    ...
    if MAX_DEPTH > 0 and _CURRENT_DEPTH + 1 > MAX_DEPTH:
        raise LokyRecursionError(
            f"Could not spawn extra nested processes at depth superior to MAX_DEPTH={MAX_DEPTH}. If this is intendend, you can change this limit with the LOKY_MAX_DEPTH environment variable."
        )
